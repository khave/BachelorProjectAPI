...
from tensorflow.keras.layers import Dense, Conv2D, Flatten, Dropout, MaxPooling2D, BatchNormalization, Rescaling, RandomFlip, RandomRotation
...
if __name__ == "__main__":
    ...
    # Global variables
    RUN_NAME = "EfficientNetV2S-Combined-Data-Aug"
    IMG_WIDTH = 224
    IMG_HEIGHT = 224
    BATCH_SIZE = 32 
    NUM_CLASSES = 17
    EPOCHS=25 # Usually write 1000, but with early stopping with for example 8 epochs with restore best weight
    ...
    # DONE Define model architecture
    
    with mlflow.start_run(run_name=RUN_NAME + "-BASE"):
        mlflow.keras.autolog()
        
        # Train model with frozen base model on epochs
        ...
        callbacks=[callbacks.EarlyStopping(patience=3, monitor='val_loss', restore_best_weights=True),
                  callbacks.ReduceLROnPlateau(patience=2, monitor='val_loss', factor=0.1, mode='auto')]
        )
    ...
    with mlflow.start_run(run_name=RUN_NAME + "-FINE_TUNED"):
        mlflow.keras.autolog()
        
        # Unfreeze the base model
        base_model.trainable = True
        
        # There are 482 layers in efficientnetv2s, so we train from layer 380
        fine_tune_at = 380
        ...
        # Recompile model so that the changes are taken into account. Remember to use even lower learning rate, as we are fine tuning now
